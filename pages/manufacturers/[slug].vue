...
import { getManufacturerBySlug, getManufacturerById, type Manufacturer } from '~/lib/services/manufacturers'
...
/** Fix errores comunes de slugs */
const fixCommonSlugIssues = (s: string) => s.startsWith('amborghini-') ? 'l' + s : s

/** Verifica si es _id de Cockpit (24 hex) */
const looksLikeId = (v: string) => /^[a-f0-9]{24}$/i.test(v)

/** ---------- Tipos ---------- */
type ViewManufacturer = Manufacturer & {
  _id?: string
  models?: string[]
...
/** ---------- Cargar fabricante (por ID o slug) ---------- */
const { data: maker, pending } = await useAsyncData<ViewManufacturer | null>(
  `manufacturer-${rawParam}`,
  async () => {
    let f: Manufacturer | null = null
    if (looksLikeId(rawParam)) {
      f = await getManufacturerById(rawParam)
      if (f) return f as ViewManufacturer
    }
    f = await getManufacturerBySlug(normalizedSlug)
    return f ? (f as ViewManufacturer) : null
  }
)

/** ---------- Modelos del fabricante ---------- */
const { data: models } = await useAsyncData<Model[]>(
  `manufacturer-models-${rawParam}`,
  async () => {
    const slugs = maker.value?.models ?? []
    if (!slugs.length) return []
    const all = await listModels({ limit: 1000 })
    return all
...
const featuredModel = computed<Model | null>(() => {
  ...
  // Si el fabricante tiene uno marcado como destacado
  ...
  // Si no, tomar el más nuevo con imagen
...
/** ---------- Imagen de logo con fallback ---------- */
const getLogo = (m: ViewManufacturer | null): string => {
  if (!m) return DEFAULT_GIF
  return (m as any).displayLogo || m.logo || DEFAULT_GIF
}
...
/** ---------- Debug ---------- */
console.log('[manufacturer-detail] rawParam:', rawParam, 'normalizedSlug:', normalizedSlug)
</script>
...
<template>
  <div class="manufacturer-detail-container">
    ...
    <!-- HERO con imagen del modelo destacado -->
    <section v-if="featuredModel?.image" class="brand-hero">
      <div class="bh-bg" :style="{ backgroundImage: `url(${featuredModel.image})` }"></div>
      <div class="bh-overlay"></div>

      <div class="bh-content">
        <div class="bh-left">
          <div class="bh-logo" v-if="maker">
            <img :src="getLogo(maker)" :alt="`Logo de ${maker.name}`" @error="handleImageError" />
          </div>
          <h1 class="bh-title">{{ maker?.name }}</h1>
          <p v-if="maker?.bio" class="bh-tag">{{ plainText(maker.bio) }}</p>
          <div class="bh-meta">
        ...
        <NuxtLink v-if="featuredModel" :to="`/models/${featuredModel._id || featuredModel.slug}`" class="bh-right">
    ...
    <!-- INFO DEL FABRICANTE -->
    <section v-if="maker" class="manufacturer-detail">
      <header class="manufacturer-header">
        <div class="manufacturer-logo-container">
          <img :src="getLogo(maker)" :alt="`Logo de ${maker.name}`" class="manufacturer-logo"
            @error="handleImageError" />
        </div>
        <div class="manufacturer-info">
          <h1 class="manufacturer-name">{{ maker.name }}</h1>
          <div class="manufacturer-meta">
            <span v-if="maker.country" class="badge">{{ maker.country }}</span>
            <span v-if="maker.founded" class="badge">Fundado: {{ maker.founded }}</span>
            <span v-if="maker.headquarters" class="badge">{{ maker.headquarters }}</span>
          </div>
          <p v-if="maker.bio" class="manufacturer-bio">{{ plainText(maker.bio) }}</p>
      ...
      <section v-if="maker.body" class="manufacturer-content">
    ...
    <div v-else class="error-state">
      ...
      <NuxtLink to="/manufacturers" class="back-button">Volver a fabricantes</NuxtLink>
    </div>
  </div>
</template>
...
.fm-cta {
  ...
  border-radius: .6rem;
  font-weight: 800;
  box-shadow: 0 10px 20px rgba(232, 61, 74, .45);
...
.bh-bg {
  ...
  animation: bh-zoom 10s ease-in-out infinite alternate
}
...
.bh-overlay {
  position: absolute;
  inset: 0;
  background: radial-gradient(120% 90% at 20% 40%, rgba(0, 0, 0, .55), rgba(0, 0, 0, .72));
}
